...
def get_IMAGENETTE(fraction, batch_size):
    transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.5,), (0.5,))])

    mnist_train = Imagenette(root='../data', split="train", download=True, transform=transform)
    mnist_test = Imagenette(root='../data', split="val", download=True, transform=transform)
